...
const domainFiltered = computed(() =>
  selectedDomain.value === "all"
    ? metrics.value
    : metrics.value.filter((m) => metricDomain(m) === selectedDomain.value)
);
...
const pathOptions = computed(() => {
  const routesInDomain =
    selectedDomain.value === "all"
      ? []
      : (appDomains.value.find((d) => d.domain === selectedDomain.value)?.routes ?? []);
  return [
    "all",
    ...Array.from(
      new Set([...routesInDomain, ...domainFiltered.value.map((m) => m.path ?? "/")])
    ).sort(),
  ];
});

const filtered = computed(() =>
  domainFiltered.value.filter((m) => {
    if (selectedPath.value !== "all" && (m.path ?? "/") !== selectedPath.value) return false;
    if (selectedDevice.value !== "all" && (m.deviceType ?? "desktop") !== selectedDevice.value)
      return false;
    return true;
  })
);
...
<template>
  <div class="wv-dashboard">
    ...
    <p class="wv-hint">
      <code>pages/</code> 1뎁스 폴더를 자동 스캔해 도메인별 추적합니다.
      <code>/series</code>, <code>/alarm</code>, <code>/my-voice</code> 등 폴더 추가 시 자동 반영.
    </p>

    <div v-if="appDomains.length" class="wv-tags">
      <span v-for="d in appDomains" :key="d.domain" class="wv-tag">{{ d.domain }}</span>
      <span v-if="scannedAt" class="wv-scanned">스캔: {{ new Date(scannedAt).toLocaleString("ko-KR") }}</span>
    </div>
    ...
    <div class="wv-filters">
      <label>도메인
        <select v-model="selectedDomain" @change="selectedPath = 'all'">
          <option value="all">전체</option>
          <option v-for="d in appDomains" :key="d.domain" :value="d.domain">{{ d.domain }}</option>
        </select>
      </label>
      <label v-if="pathOptions.length > 1">페이지
        <select v-model="selectedPath">
          <option v-for="p in pathOptions" :key="p" :value="p">{{ p === "all" ? "전체" : p }}</option>
        </select>
      </label>
      <label>기기
    ...
    <div class="wv-cards">
      <div v-for="name in order" :key="name" class="wv-card">
        <div class="wv-card-label">{{ name }}</div>
        <div class="wv-card-value">
          {{ latestByType[name] ? `${formatValue(name, latestByType[name].value)}${UNIT[name]}` : "—" }}
        </div>
      </div>
    </div>
    ...
    <table v-if="filtered.length" class="wv-table">
      <thead>
        <tr>
          <th>도메인</th><th>페이지</th><th>기기</th><th>지표</th><th>값</th><th>수신</th>
        </tr>
      </thead>
      <tbody>
        <tr v-for="(m, i) in filtered.slice(0, 50)" :key="`${m.id}-${i}`">
          <td>{{ metricDomain(m) }}</td>
          <td>{{ m.path ?? "/" }}</td>
          <td>{{ DEVICE_LABEL[(m.deviceType ?? "desktop") as DeviceType] }}</td>
          <td>{{ m.name }}</td>
          <td>{{ formatValue(m.name, m.value) }}{{ UNIT[m.name] }}</td>
          <td>{{ new Date(m.receivedAt).toLocaleString("ko-KR") }}</td>
        </tr>
      </tbody>
    </table>
...
.wv-scanned { font-size: 0.75rem; color: #a1a1aa; align-self: center; }
.wv-filters { display: flex; flex-wrap: wrap; gap: 1rem; margin: 1rem 0; }
.wv-cards { display: grid; grid-template-columns: repeat(auto-fit, minmax(8rem, 1fr)); gap: 1rem; margin: 1.5rem 0; }
.wv-card { border: 1px solid #e4e4e7; border-radius: 0.75rem; padding: 1rem; }
.wv-card-label { font-size: 0.875rem; color: #71717a; }
.wv-card-value { font-size: 1.5rem; font-weight: 600; }
...
.wv-empty { color: #71717a; padding: 1rem; border: 1px solid #e4e4e7; border-radius: 0.5rem; }
button { background: #18181b; color: #fff; border: none; border-radius: 0.5rem; padding: 0.5rem 1rem; cursor: pointer; }
